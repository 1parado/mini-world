// HighlighterTool — with real-time preview + colorIndex wiring
...
export class HighlighterTool {
  ...
  finishStroke() {
    ...
    if (this.currentPoints.length >= 2) {
      const stroke = { points: this.currentPoints.slice(), color: this.color, width: 16, tool: 'highlighter' };
  ...
  _renderPreview() {
    ...
    const page = this.currentSide === 'left'
      ...
      : this.notebook.getCurrentRightPage();
    if (!page) return;
    const tmpStroke = { points: this.currentPoints.slice(), color: this.color, width: 16, tool: 'highlighter' };
